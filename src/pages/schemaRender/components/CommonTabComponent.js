...
import { formFieldValidationAction, productFormFieldUpdateAction } from 'src/slice/schema';
...
const CommonTabComponent = (props) => {
    ...
    const productState = useSelector(state => state.schema);
    ...
    const getSelectedProductFormLanguage_value = () => {
        const languageListValue = {
            formLanguageList: [{ label: "English", value: "en" },{label:"Finnish",value:"fi"}],
                ...
                <>
                    ...
                    >
                        ...
                        {handleShowSectionLoading() &&
                            <div className="spinner-border spinner-border-sm text-danger ms-2" role="status"></div>
                        }
                    </div>
                </>
            )
        ...
        <div className='row mb-5 pb-5'>
            {formFieldList && formFieldList[0] && formFieldList.map((item, index) => (
                <React.Fragment key={`formFieldSchemaRender_main_${item.name}_${index}`}>
                    ...
                    <ErrorBoundary key={`formFieldSchemaRender_main_errorboundary_${item.name}_${index}`}>
