...
export function InputDateTimePicker(props) {
  ...
  return (
    <div
      className={`form-group inputDateTimePicker col-lg-6 col-md-6 col-sm-6 col-xs-6 ${
        className ? className : ""
      }`}
    >
      <div className="col inputTextWithLabel__label">
        ...
        {tooltipText && <CustomTooltip tooltipContent={<p>{tooltipText}</p>} />}
      </div>
      <div>
        ...
        <div className="text-danger">{(isAllTouched || touched) && error}</div>
      </div>
    </div>
  );
}
