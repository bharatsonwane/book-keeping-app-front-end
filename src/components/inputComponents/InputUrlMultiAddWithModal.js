...
import FormFieldSchemaRender from "../schemaRender/FormFieldSchemaRender";
import { getValidationErrorObjectForYup } from "src/helper/validationYupHelper";
import { socialMediaUrlTypes } from "src/helper";
...
export function InputUrlMultiAddWithModal(props) {
  ...
  let selectedSchema;
  if (socialMediaUrlTypes && socialMediaUrlTypes[0] && label ==="Social media URL") {
    const newOptions = socialMediaUrlTypes?.map((item) => {
      const matchObject =
        value &&
        value[0] &&
    ...
    <>
      ...
      >
        ...
        <div className="inputUrlMultiAddWithModal__value">
          <div className="col-12 ps-md-5 pe-5">
            <div className="row">
              {value && value[0] ? (
