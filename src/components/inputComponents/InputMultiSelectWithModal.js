...
export function InputMultiSelectWithModal(props) {
  ...
  useEffect(() => {
    let uniqueValuesArray = _.uniq(value);
    setModalData({
      isShow: false,
      multiSelectValue: uniqueValuesArray
    })
  }, [value])
  ...
  const handleRemoveItem = (item) => {
    const newValue = modalData?.multiSelectValue?.filter((element) => element !== item)
    onChange({
      target: {
        name: name,
        value: newValue,
    ...
    <>
      <div className={`form-group inputMultiSelectWithModal col-lg-12 col-md-12 col-sm-12 col-xs-6 ${className ? className : ""}`}>
        ...
        <div className="inputMultiSelectWithModal__value">
          {modalData?.multiSelectValue && modalData?.multiSelectValue?.[0] ?
            modalData?.multiSelectValue?.map((item, index) => (
            ...
            :
            <>
              <span className="passive-message me-2">{t("No data found")}</span>
            </>
          }
          {
            !readOnly &&
