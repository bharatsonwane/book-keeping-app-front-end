 ...
 */
export const getTabDataAndParentTabNameByName = (schema, tabName, parentTabName = null) => {
    ...
    if (schema.children && Array.isArray(schema.children)) {
      for (const child of schema.children) {
        ...
        const result = getTabDataAndParentTabNameByName(child, tabName, schema.tabName);
        if (result?.tabData) {
...
export const getInitialTabName = (schema) => {
  if (schema.tabName && schema?.childrenType !== "childTab") {
