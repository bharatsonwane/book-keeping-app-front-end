  ...
  return (
    <>
      <div className="d-flex align-content-start  h-100">
        <nav
          ref={ref}
          className={`h-100 sidebar ${getThemedTextColor()} ${getThemedBgColor()} p-3`}
        >
          <a
            href="/"
                ...
                return (
                  <li key={item.key}>
                    <NavLink
                      type="button"
                      to={"#"}
                      className={`nav-link text-start ${getThemedTextColor()} dropdown-navlink dropdown-toggle  ${
                        item.vissbleOnSmallScrren ? "" : "d-none d-md-block"
                      }`}
                      aria-current="page"
